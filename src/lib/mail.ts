...
export async function sendOutreachEmail(params: OutreachEmailParams) {
    ...
    const htmlContent = `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #334155;">
            <h2 style="color: #ea580c;">Oportunidade de Inovação: ${researchTitle}</h2>
            ...
            <p>Acreditamos que esta solução pode agregar valor aos seus objetivos estratégicos. Gostaríamos de agendar uma breve conversa de 15 minutos para apresentar os detalhes técnicos e as possibilidades de licenciamento ou parceria.</p>
    ...
    try {
        const data = await resend.emails.send({
            from: 'Inovação <onboarding@resend.dev>', // Should be a verified domain in production
            to: [to],
            subject: subject,
            html: htmlContent,
        });
