    ...
    try {
        ...
        if (existingPipeline) {
            ...
            await supabase.from("stages").insert([
                ...
                { pipeline_id: pipelineId, name: "Qualificação", color: "bg-blue-100", order: 1 },
                { pipeline_id: pipelineId, name: "NDA / Negociação", color: "bg-orange-100", order: 2 },
        ...
        if ((companyCount || 0) === 0) {
            const companies = [
                ...
                { name: "Siemens Brasil", industry: "Automação / IoT", description: "Especialista em sensores industriais, infraestrutura IoT e automação de processos. Investindo em edge computing e digital twins para indústria 4.0." },
                ...
                { name: "Embraer X", industry: "Aeroespacial / Mobilidade", description: "Braço de inovação da Embraer focado em mobilidade aérea urbana, materiais compostos avançados e propulsão elétrica para aviação." },
                { name: "Vale S.A.", industry: "Mineração / Descarbonização", description: "Maior mineradora do Brasil com programa de descarbonização. Busca tecnologias para captura de carbono, reabilitação ambiental e automação de minas." },
                ...
                { name: "TOTVS", industry: "Tecnologia / Software", description: "Maior empresa de tecnologia do Brasil. Investindo em IA aplicada a gestão empresarial, cloud computing e plataformas de dados." },
        ...
        if ((contactCount || 0) === 0) {
            const decisionMakers = [
                { name: "Dr. Ricardo Almeida", company: "Natura &Co", role: "Diretor de Inovação e P&D", email: "r.almeida@natura.example.com", expertise: ["Biotecnologia", "Sustentabilidade"], verified: true, type: "lead", status: "qualified" },
                ...
                { name: "Marcos Pontes Jr.", company: "Vale S.A.", role: "Head de Open Innovation", email: "m.pontes@vale.example.com", expertise: ["Descarbonização", "Automação"], verified: true, type: "lead", status: "new" },
                ...
                { name: "Prof. Carlos Mendes", company: "Braskem", role: "Diretor de Inovação Sustentável", email: "c.mendes@braskem.example.com", expertise: ["Bioplásticos", "Economia Circular"], verified: true, type: "lead", status: "new" },
                { name: "Fernanda Lima", company: "Siemens Brasil", role: "Head of Digital Industries", email: "f.lima@siemens.example.com", expertise: ["IoT", "Automação Industrial"], verified: true, type: "lead", status: "contacted" },
        ...
        if ((researcherCount || 0) === 0) {
            const researchers = [
                ...
                { name: "Prof. Dr. André Oliveira", department: "Engenharia Elétrica", expertise: ["IoT", "Sensores", "Automação"], publications_count: 35, patents_count: 2, h_index: 9, lattes_id: "1122334455" },
                { name: "Profa. Dra. Luciana Martins", department: "Ciências Agrárias", expertise: ["Bioinsumos", "Agricultura de Precisão"], publications_count: 61, patents_count: 4, h_index: 15, lattes_id: "5566778899" },
                { name: "Prof. Dr. Fernando Rocha", department: "Ciência da Computação", expertise: ["IA", "Machine Learning", "NLP"], publications_count: 29, patents_count: 1, h_index: 8, lattes_id: "9988776655" },
        ...
        if ((grantCount || 0) === 0) {
            const grants = [
                { name: "Edital Finep - Mais Inovação Brasil", agency: "Finep", value: "Até R$ 10.000.000", deadline: "Fluxo Contínuo", description: "Apoio a projetos de inovação em empresas brasileiras. Foco em tecnologias críticas e soberania nacional.", url: "https://finep.gov.br", relevance_score: 0.92 },
                ...
                { name: "BNDES Fundo Tecnológico (FUNTEC)", agency: "BNDES", value: "Sob Consulta (R$ 5M+)", deadline: "Aberto permanente", description: "Apoio financeiro a projetos de P&D que visem inovação de alto impacto no setor produtivo brasileiro.", url: "https://bndes.gov.br", relevance_score: 0.88 },
                ...
                { name: "FAPESP - PIPE (Pesquisa Inovativa)", agency: "FAPESP", value: "Até R$ 1.200.000", deadline: "Fluxo Contínuo", description: "Apoio à pesquisa inovativa em pequenas empresas do Estado de São Paulo. Fases I, II e III.", url: "https://fapesp.br", relevance_score: 0.82 }
            ];
        ...
        if ((campaignCount || 0) === 0) {
            await supabase.from("outreach_campaigns").insert([
                { organization_id: orgId, user_id: user.id, name: "Prospecção: Patente Grafeno V2", status: "active", target_technology: "Grafeno", total_sent: 128, total_opened: 84, total_clicked: 32, total_replied: 12 },
        ...
        if ((auditCount || 0) === 0) {
            const auditEntries = [
                ...
                { action: "SEED_DATA", entity_type: "system", details: { event: "Dados de demonstração inseridos" } },
